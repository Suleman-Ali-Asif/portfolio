...
**Next.js 15 App Router** portfolio site using React 19, Tailwind CSS v4, Framer Motion, `next-themes`, and three.js (one component only, lazy loaded).
...
- `src/app/component/`
  ...
  - `DotLaptop.tsx` / `DotLaptopScene.tsx` — the intro tile's ornament: an open laptop drawn entirely in full stops, as one `Points` cloud on a regular grid (deck plane plus lid plane hinged at its back edge). Shapes come from per-dot alpha, never from moving a dot, so the grid stays regular; no font, model or texture asset. A custom `ShaderMaterial` does the work: dots near the cursor lift toward the camera and spread apart (scaled by `1 - (r/R)^2`, which hits zero at the rim, so there is no seam), which is the hover magnifier. "VINI VIDI VICI" is hidden in the screen's dot field as 5x5 bitmaps on the same grid (`GLYPHS` / `lidText`) and lights up only while the pointer is inside the projected screen quad, seen from the front (`onScreen()`, a winding test); the field dims and the magnifier stands down while the words are up, or the bulge would scramble them. Hover also leans it toward the pointer, drag turns it, a throw coasts, and at rest it sways around a three-quarter pose. Dot colour is the `--text` token, re-read on a theme flip, with a little extra weight on paper. `DotLaptop` lazy-loads the scene (`next/dynamic`, `ssr: false`) so three.js stays out of the first load, showing a dotted-stroke SVG laptop until then, or for good if WebGL is missing. The loop runs only while the tile is on screen in a visible tab; `prefers-reduced-motion` parks it, keeping only the pointer-driven magnifier and message, which are direct manipulation
